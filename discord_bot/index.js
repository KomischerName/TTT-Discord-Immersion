...
client.on('ready', () => {
    ...
    channel = guild.channels.resolve(config.discord.channel);
    //	channel = guild.channels.find('id',config.discord.channel);
});
client.on('voiceStateUpdate', (oldState, newState) => {//player leaves the ttt-channel
    if (oldState.channel != newState.channel && isMemberInVoiceChannel(oldState.member)) {
        if (isMemberMutedByBot(newState.member) && newState.serverMute) newState.setMute(false).then(() => {
            setMemberDeafenedByBot(newState.member, false);
        });
    }
});

isMemberInVoiceChannel = (member) => member.voice.channelID == config.discord.channel;
...
get['connect'] = (params, ret) => {
    ...
    let found = guild.members.cache.filter(val => val.user.tag.match(new RegExp('.*' + tag + '.*'))).array();
    if (found.length > 1) {
        ret({
